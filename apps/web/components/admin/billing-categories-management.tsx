...
export function BillingCategoriesManagement() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const categoryResponse = await listBillingCategories({ includeInactive: true });
        setCategories(categoryResponse);
      } catch (loadError) {
        setError(loadError instanceof Error ? loadError.message : "Unable to load billing categories.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    async function loadCategories() {
      ...
      try {
        ...
        if (editingCategoryId && !response.some((entry) => entry.id === editingCategoryId)) {
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Unable to create billing category.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleSaveEdit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const next = updated.find((entry) => entry.id === editingCategoryId) ?? null;
      ...
      setError(saveError instanceof Error ? saveError.message : "Unable to update billing category.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleArchive(category: BillingCategory) {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : "Unable to archive billing category.");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    return (
      <Card>
        <CardContent className="pt-6">
          <p className="text-sm text-slate-500">Loading billing categories...</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter category list by school and status.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <Field htmlFor="billing-categories-include-inactive" label="Status scope">
            <Select
      ...
      {canManage ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add a new billing category for a school.</CardDescription>
          </CardHeader>
          <CardContent>
            <form className="grid gap-4 md:grid-cols-4" onSubmit={handleCreate}>
              ...
              <Field htmlFor="create-billing-category-description" label="Description (optional)">
                <Input
      ...
      {canManage && editingCategory ? (
        <Card>
          ...
          <CardContent>
            <form className="grid gap-4 md:grid-cols-3" onSubmit={handleSaveEdit}>
              ...
              <Field htmlFor="edit-billing-category-description" label="Description (optional)">
                <Input
