  ...
  return (
    <Card>
      <CardContent className="pt-6">
        ...
        <p
          className={`text-2xl font-bold tabular-nums ${tone === "danger" ? "text-red-600" : "text-slate-900"}`}
        >
          {value}
        </p>
      </CardContent>
...
export function BillingReportsManagement() {
  ...
  const [paymentsReport, setPaymentsReport] =
    useState<BillingPaymentsReport | null>(null);
  const [chargesReport, setChargesReport] =
    useState<BillingChargesReport | null>(null);
  const [outstandingReport, setOutstandingReport] =
    useState<BillingOutstandingReport | null>(null);
  const [summaryReport, setSummaryReport] =
    useState<BillingSummaryReport | null>(null);
  ...
  useEffect(() => {
    ...
    if (
      !filteredStudents.some(
        (student) => student.id === paymentsFilters.studentId,
      )
    ) {
    ...
    if (
      !filteredStudents.some(
        (student) => student.id === chargesFilters.studentId,
      )
    ) {
      setChargesFilters((current) => ({ ...current, studentId: "" }));
    }
  }, [
    chargesFilters.studentId,
    filteredStudents,
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Apply a school filter across all billing reports on this page.
          </CardDescription>
        </CardHeader>
        <CardContent className="max-w-sm">
          <Field htmlFor="billing-reports-school" label="School">
            <Select
              id="billing-reports-school"
              value={schoolId}
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Headline totals for charges, payments, and current balances.
              </CardDescription>
            </div>
            <Button
          ...
          {isLoadingSummary || !summaryReport ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
              <SummaryMetric
                ...
                value={formatCurrency(summaryReport.totalChargesIssued)}
              />
              <SummaryMetric
                label="Payments Received"
                value={formatCurrency(summaryReport.totalPaymentsReceived)}
              />
              <SummaryMetric
                label="Voided Payments"
                value={formatCurrency(summaryReport.totalVoidedPayments)}
              />
              <SummaryMetric
                label="Current Outstanding"
                value={formatCurrency(summaryReport.currentOutstanding)}
                tone={
                  isPositiveAmount(summaryReport.currentOutstanding)
                    ? "danger"
                    : undefined
                }
              />
              <SummaryMetric
                ...
                tone={
                  isPositiveAmount(summaryReport.currentOverdue)
                    ? "danger"
                    : undefined
                }
              />
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Payment activity by receipt, student, method, and status.
              </CardDescription>
            </div>
            <Button
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <Field htmlFor="payments-from" label="Date from">
              <Input
                id="payments-from"
                type="date"
                value={paymentsFilters.dateFrom}
                onChange={(event) =>
                  setPaymentsFilters((current) => ({
                    ...current,
                    dateFrom: event.target.value,
                  }))
                }
            ...
            <Field htmlFor="payments-student" label="Student">
              <Select
                id="payments-student"
                value={paymentsFilters.studentId}
                onChange={(event) =>
                  setPaymentsFilters((current) => ({
                    ...current,
            ...
            <Field htmlFor="payments-include-voided" label="Include voided">
              <Select
                id="payments-include-voided"
                value={paymentsFilters.includeVoided ? "true" : "false"}
                onChange={(event) =>
                  setPaymentsFilters((current) => ({
                    ...current,
                    includeVoided: event.target.value === "true",
                  }))
                }
              >
          ...
          {isLoadingPayments || !paymentsReport ? (
            ...
            <EmptyState
              ...
              description="No payment records matched the selected filters."
            />
          ) : (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-3 text-sm text-slate-600">
                <Badge variant="neutral">
                  {paymentsReport.totals.count} rows
                </Badge>
                <Badge variant="neutral">
                  Total {formatCurrency(paymentsReport.totals.totalAmount)}
                </Badge>
              </div>
                  ...
                  <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                      {paymentsReport.items.slice(0, 20).map((item) => (
                        <tr
                          key={`${item.receiptNumber}-${item.paymentDate}`}
                          className="hover:bg-slate-50"
                        >
                          <td className="px-4 py-3 text-slate-700">
                            {formatDateLabel(item.paymentDate)}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <Badge
                              variant={
                                item.status === "VOIDED" ? "danger" : "success"
                              }
                            >
                              {item.status}
                            </Badge>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Charges issued by date, student, category, and status.
              </CardDescription>
            </div>
            <Button
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <Field htmlFor="charges-from" label="Date from">
              <Input
                id="charges-from"
                type="date"
                value={chargesFilters.dateFrom}
                onChange={(event) =>
                  setChargesFilters((current) => ({
                    ...current,
                    dateFrom: event.target.value,
                  }))
                }
            ...
            <Field htmlFor="charges-category" label="Category">
              <Select
                id="charges-category"
                value={chargesFilters.categoryId}
                onChange={(event) =>
                  setChargesFilters((current) => ({
                    ...current,
            ...
            <Field htmlFor="charges-student" label="Student">
              <Select
                id="charges-student"
                value={chargesFilters.studentId}
                onChange={(event) =>
                  setChargesFilters((current) => ({
                    ...current,
                    studentId: event.target.value,
          ...
          {isLoadingCharges || !chargesReport ? (
            ...
            <EmptyState
              compact
              title="No charge rows"
              description="No charges matched the selected filters."
            />
          ) : (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-3 text-sm text-slate-600">
                <Badge variant="neutral">
                  {chargesReport.totals.count} rows
                </Badge>
                <Badge variant="neutral">
                  Issued {formatCurrency(chargesReport.totals.totalAmount)}
                </Badge>
                <Badge variant="neutral">
                  Due {formatCurrency(chargesReport.totals.totalDue)}
                </Badge>
              </div>
              <div className="overflow-hidden rounded-xl border border-slate-200">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                      {chargesReport.items.slice(0, 20).map((item) => (
                        <tr
                          key={`${item.issuedAt}-${item.studentName}-${item.title}`}
                          className="hover:bg-slate-50"
                        >
                          <td className="px-4 py-3 text-slate-700">
                            {formatDateLabel(item.issuedAt)}
                          </td>
                          <td className="px-4 py-3 text-slate-700">
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Student-level outstanding and overdue balances.
              </CardDescription>
            </div>
            <Button
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <Field htmlFor="outstanding-min-balance" label="Min balance (CAD)">
              <Input
                id="outstanding-min-balance"
                type="number"
                value={outstandingFilters.minBalance}
                onChange={(event) =>
                  setOutstandingFilters({ minBalance: event.target.value })
                }
                placeholder="e.g. 100"
              />
            </Field>
          ...
          {isLoadingOutstanding || !outstandingReport ? (
            ...
            <EmptyState
              compact
              title="No outstanding rows"
              description="No students matched the outstanding balance filters."
            />
          ) : (
            <div className="space-y-3">
              <div className="flex flex-wrap gap-3 text-sm text-slate-600">
                <Badge variant="neutral">
                  {outstandingReport.totals.studentCount} students
                </Badge>
                <Badge variant="neutral">
                  Outstanding{" "}
                  {formatCurrency(outstandingReport.totals.totalOutstanding)}
                </Badge>
                <Badge variant="neutral">
                  Overdue{" "}
                  {formatCurrency(outstandingReport.totals.totalOverdue)}
                </Badge>
              </div>
              <div className="overflow-hidden rounded-xl border border-slate-200">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                      {outstandingReport.items.slice(0, 20).map((item) => (
                        <tr
                          key={`${item.schoolId}-${item.studentId}`}
                          className="hover:bg-slate-50"
                        >
                          <td className="px-4 py-3 font-medium text-slate-900">
                            {item.studentName}
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-2">
                              <Link
                                ...
                                href={`/admin/billing/students/${encodeURIComponent(item.studentId)}`}
                              >
                                View account
                              </Link>
                              <Link
                                ...
                                href={`/admin/billing/payments/new?studentId=${encodeURIComponent(item.studentId)}`}
                              >
                                Record payment
                              </Link>
                            </div>
